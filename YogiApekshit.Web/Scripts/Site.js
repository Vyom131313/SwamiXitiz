function documentReady(root, controller, action) {
    ...
    $('#chTheme').change(function () {
        ...
        $('#aweStyle').attr('href', root + "Content/themes/" + theme + "/IConweyMvc.css?v=83");
...
function deleteFormat(popupName, gridId) {
    return function (model) {
        if (model.IsDeleted) {
            return "<button type='button' class='awe-btn' style='padding:0px;margin:0px;width:25px;height:25px;border:1px solid red;background-color:transparent' onclick=\"restore('" + gridId + "'," + model.Id + ")\"><span class='ico-restore'></span></button>";
        }
...
function statusColumnFormat() {
    return function (model) {
        if (model.IsActive)
            return "<img src='./Content/Images/active.png' width='16px' height='16px' style='align:right' />";
        else
            return "<img src='./Content/Images/inactive.png' width='16px' height='16px' style='align:right'/>";
    };
}
